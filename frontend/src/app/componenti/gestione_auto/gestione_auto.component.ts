...
export class Gestione_autoComponent implements OnInit {
  ...
  eliminaAuto(): void {
    const autoId = this.formAggiungiAuto.value.deleteId;

    if (autoId) {
      this.service.deleteAutoById(autoId).subscribe({
        next: (response) => {
      ...
      console.error('Nessun ID auto fornito per l\'eliminazione.');
    }
  }
  ...
  calculateSuccessProbability(): void {
    ...
    // **Anno**
    const avgYear = Math.floor(filteredAutos.reduce((sum, auto) => sum + auto.anno, 0) / filteredAutos.length);
